...
p = Popen(['source_code/benchmarkD', '--file', path+"/automaton", '--n', str(n), '--l', str(lower_bound), '--u', str(upper_bound)])
p.wait()
...
print('write result into ', path_evaluation +'/benchmarkD.csv')
## run evaluation
with open(path_evaluation+'/benchmarkD.csv', mode='w') as out_csv:
