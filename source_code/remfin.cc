...
namespace spot
{
  namespace
  {
    ...
    twa_graph_ptr mod_default_strategy(const const_twa_graph_ptr& aut)
    {
      std::vector<acc_cond::acc_code> code; // acc condition for each Fin-Block (without Fin)
      std::vector<acc_cond::mark_t> rem; // vector of labels that are removed for each Fin-block
      std::vector<acc_cond::mark_t> keep; // vector of labels that remain for each Fin-block
      std::vector<acc_cond::mark_t> add; // labels that are aded for each Fin-block
      bool has_true_term = false;
      acc_cond::mark_t allinf = {};
      acc_cond::mark_t allfin = {};
      {
      ...
      bool interference = false;
      {
       ...
       // if (interference) OLD
        if (false)
          {
            std::cout << "We have interferences\n";
            // We need extra set, but we will try
            // to reuse the Fin number if they are
            // not used as Inf as well.
            std::vector<int> exs(acc.num_sets());
        ...
        else if (has_true_term) //OLD
        //else if (false)
          {
            //std::cout << "We have a true term\n";
            unsigned one = acc.add_sets(1);
            extra_sets += 1;
            acc_cond::mark_t m({one});
            auto c = acc.inf(m);
      ...
      unsigned cs = code.size(); // number of Fin-blocks after all the proprocessing is done
      for (unsigned i = 0; i < cs; ++i)
       // std::cout << i << " Rem " << rem[i] << "  Code " << code[i]
       //       << " Keep " << keep[i] << " Add " << add[i] <<'\n';
        trace << i << " Rem " << rem[i] << "  Code " << code[i]
              << " Keep " << keep[i] << '\n';

      unsigned nst = aut->num_states();
      auto res = make_twa_graph(aut->get_dict()); // resulting, new automaton
      res->copy_ap_of(aut);
      res->prop_copy(aut, { true, false, false, false, false, true });
      res->new_states(nst);
      res->set_acceptance(aut->num_sets() + extra_sets, new_code);
      ...
      bool sbacc = res->prop_state_acc().is_true();
     // std::cout<< "sbacc: "<<sbacc << std::endl;
      scc_info si(aut, scc_info_options::TRACK_STATES);
      unsigned nscc = si.scc_count();

      // NEW STUFF
      // calculate how many inf-sets we need
      unsigned max_inf_sets = 0; // number of Inf-sets we will need for the automaton

      // if we have a true term, we introduced a new inf-set and therefore need at least one set
      if (has_true_term)
        max_inf_sets = 1;

      for (auto c : code)
      {
        for (auto acc : c.top_disjuncts())
        {
          if (acc.used_sets().count() > max_inf_sets)
            max_inf_sets = acc.used_sets().count();
        }
      }
     // std::cout << "max_inf_sets: " << max_inf_sets << std::endl;
      acc_cond::mark_t all_lables = {}; // all inf-labels that the automaton needs
      // add all the labels until we reach the myximum number of sets
      for (unsigned i = 0; i < max_inf_sets; ++i)
      {
        all_lables.set(i);
      }
     // std::cout << "all_labels: " << all_lables << std::endl;
      ...
      for (unsigned n = 0; n < nscc; ++n)
        {
          auto m = si.acc_sets_of(n); // all labels in this SCC
          auto states = si.states_of(n); // states in this SCC
          trace << "SCC #" << n << " uses " << m << '\n';
          //std::cout << "SCC #" << n << " uses " << m << '\n';
          ...
          //std::cout << "main_sets " << main_sets << "\nmain_add "
           //     << main_add << '\n';

          // If the SCC is rejecting, there is no need for clone.
          // Pretend we don't interesect any Fin.
          bool is_rejecting = false; // wether the scc is rejecting --> no clone needed
          if (si.is_rejecting_scc(n))
            //intersects_fin = false; OLD
            is_rejecting = true;
          ...
          for (auto s: states)
            for (auto& t: aut->out(s))
              {
                acc_cond::mark_t a = {};
                // if (sbacc || SPOT_LIKELY(si.scc_of(t.dst) == n)) OLD but useful
                  //a = (t.acc & main_sets) | main_add; // OLD
                 // a = main_add; // NEW
          ...
          for (unsigned i = 0; i < cs; ++i)
           ...
           // if (m & rem[i]) OLD
              ...
              auto top_dis = code[i].top_disjuncts(); // the top disjuncts of the acc code in this SCC

              for (auto acc : top_dis)
              {
                //std::cout<< "acc: " << acc << " used sets: " << acc.used_sets() << " m: " << m <<std::endl;
                //auto l = acc.used_sets(); // all the lables that occur in the recent condition
                // make copy if we have all labels of the accepting condition in the SCC
                auto m_and_add = (m | add[i]);
                //std::cout << "m: " << m << " add[i]: " << add[i] << " acc.used_sets: " << acc.used_sets() << std::endl;
                if (acc.used_sets().subset(m_and_add))
                {
                  //std::cout << "make copy for " << acc << std::endl;
                  // create the mapping for the used sets
                  std::map<unsigned, unsigned> new_label; // mapping of old labels to new ones
                  
                  auto used_sets = acc.used_sets();
                  unsigned j = 0;
                  for (auto label: used_sets.sets())
                  {
                    new_label.insert(std::make_pair(label, j++));
                  }

                  
                  //mapping additional labels to new ones
                  for (auto label : add[i].sets())
                  {
                    if (!used_sets.has(label))
                       new_label.insert(std::make_pair(label, j++));
                  }

                  // calculate the sets that are added to each transition
                  acc_cond::mark_t a = {};
                  for (;j < max_inf_sets; ++j)
                    a.set(j);
                  //std::cout<< "a: " << a << std::endl;

                  
                  // set the additional labels for all edges
                  for (auto label : add[i].sets())
                  {
                    a.set(new_label[label]);
                  }

                 // std::cout<< "a: " << a << std::endl;
                  auto r = rem[i];
                  //std::cout << "rem[" << i << "] = " << r << " requires a copy\n";
                  unsigned base = res->new_states(states.size());
                  for (auto s: states)
                    state_map[s] = base++;
                  //auto k = keep[i]; old
                  ...
                  for (auto s: states)
                    {
                      ...
                      for (auto& t: aut->out(s))
                        {
                          acc_cond::mark_t t_new_label = {}; // new label of the transition t
                          for (auto l : (t.acc & acc.used_sets()).sets())    
                              t_new_label.set(new_label[l]);
                          if (unsigned en = aut->edge_number(t);
                              // Ignore edges have r, or that
                              // necessarily go through r.
                            // ((t.acc | propmarks[en]) & r) OLD 
                            (t.acc & r)
                              // Ignore edges between SCCs.
                             || si.scc_of(t.dst) != n )
                              // Ignore edges that are already accepting
                            ...
                            continue;
                          auto nd = state_map[t.dst];
                          //std::cout<<"ns: "<<ns  << "  nd: "<<nd<<std::endl;
                         // res->new_edge(ns, nd, t.cond, ((t.acc & k) | a) & l); OLD
                         // if (t_new_label.count() > 0 || acc.used_sets().count() == 0) if the additional labels should not be set on every transition => prob. worse performance
                            res->new_edge(ns, nd, t.cond, (t_new_label  | a));
                         // else
                         //   res->new_edge(ns, nd, t.cond, t_new_label );
                          ...
                          if (t.dst <= s)
                            {
                             ...
                             // if (sbacc) OLD
                                //a = (t.acc & main_sets) | main_add; OLD
                              //res->new_edge(s, nd, t.cond, a & l); OLD
                              res->new_edge(s, nd, t.cond, a );
                              //std::cout << "s: "<< s << " nd: " << nd <<std::endl;
                            }
                        }
                    }
                }
      ...
      res->set_generalized_buchi(max_inf_sets);
      res->purge_dead_states();
      trace << "before cleanup: " << res->get_acceptance() << '\n';
      cleanup_acceptance_here(res);
      trace << "after cleanup: " << res->get_acceptance() << '\n';
  ...
  twa_graph_ptr remove_fin2314(const const_twa_graph_ptr& aut)
  {
...
namespace mod_spot 
{
 int test_function(int in)
  {
    return in+30;
  }

  spot::twa_graph_ptr remove_fin(const spot::const_twa_graph_ptr& aut)
  {
    if (aut->get_acceptance().to_dnf().top_disjuncts().size() > 1)
    {
      spot::twa_graph_ptr res = spot::default_strategy(aut);
      assert(!res->acc().uses_fin_acceptance());
      assert(!res->acc().is_f());
      return res;
    }
    ...
    {
      // our algorihtm is useless
      return spot::remove_fin(aut);
    }
  }

  spot::twa_graph_ptr remove_fin_and_generalize(const spot::const_twa_graph_ptr& aut)
  {
    if (aut->get_acceptance().to_dnf().top_disjuncts().size() > 1)
    {
      //std::cout<< "wrong" <<std::endl;
      spot::twa_graph_ptr res = spot::mod_default_strategy(aut);
      assert(!res->acc().uses_fin_acceptance());
      assert(!res->acc().is_f());
      return res;
    }
    else
    {
     // std::cout <<"right" << std::endl;
      auto res = spot::remove_fin(aut);
      return spot::to_generalized_buchi(res);
    }
  }
}